...
def main_users() -> None:
    ...
    print(post('http://localhost:5000/api/v2/users').json())  # нет данных
    print(post('http://localhost:5000/api/v2/users', json={'name': 'Sonya'}).json())  # не все поля
    print(post('http://localhost:5000/api/v2/users', json={'name': 'Sonya',
                                                           'position': 'junior programmer',
                                                           'surname': 'Wolf',
                                                           'age': 17, 'address': 'module_3',
                                                           'speciality': 'computer sciences', 'password': 'wolf',
                                                           'email': 'wolf@mars.org', })
          .json())
    print(delete('http://localhost:5000/api/v2/users/999').json())  # id = 999 нет в базе
    # print(delete('http://localhost:5000/api/v2/users/8').json())
